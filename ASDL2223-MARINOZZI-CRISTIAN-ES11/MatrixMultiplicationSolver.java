...
public class MatrixMultiplicationSolver {
	...
	/**
	 * Costruisce un solver per una certa sequenza di matrici da moltiplicare, date
	 * le loro dimensioni righeXcolonne. Il calcolo della soluzione ottima viene
	 * eseguito subito, cioè come parte di questo costruttore.
	 * 
	 * @param p è una lista di valori che sono le dimensioni delle matrici, ad
	 *          esempio se p = [10, 100, 5, 50] allora sto moltiplicando 3 matrici
	 *          (p.size() - 1) le cui dimensioni sono A_{0} = 10x100, A_{1} = 100x5,
	 *          A_{2} = 5x50
	 * @throws NullPointerException     se la lista passata è null
	 * @throws IllegalArgumentException se la lista p contiene meno di due elementi
	 *                                  (cioè deve contenere almeno una matrice. Nel
	 *                                  caso di una unica matrice la soluzione è 0 e
	 *                                  la parentesizzazione è la matrice stessa,
	 *                                  cioè "A_{0}")
	 */
	...
	private void solve() {
		int n = p.size() - 1;
		/*
		 * Inizializzo la diagonale principale uguale a 0
		 */
		for (int i = 0; i < n; i++) {
			m[i][i] = 0;
		}
	...
	/**
	 * Restituisce il numero minimo di moltiplicazioni necessarie per moltiplicare
	 * la sequenza di matrici di questo solver. Nel caso di una sola matrice
	 * restituisce zero.
	 *
	 ...
	 * dimensione p.get(i) x p.get(i+1) con 0 <= i <= p.size() - 2. Ad esempio la
	 * parentesizzazione con una sola matrice deve restituire "A_{0}", la
	 * parentesizzazione con due matrici deve restituire "(A_{0} x A_{1})", la
	 * parentesizzazione con tre matrici deve restituire "((A_{0} x A_{1}) x A_{2})"
	 * oppure "(A_{0} x (A_{1} x A_{2}))" e così via.
	 *
	...
	private String traceBack(int i, int j) {
		/*
		 * Se la parentesizzazione contiene una sola matrice, restituisco "A_{i}".
		 */
		if (i == j) {
			return "A_{" + i + "}";
			/*
			 * Se la parentesizzazione contiene due matrici, restituisco "(A_{i} x A_{j})".
			 */
		} else {
