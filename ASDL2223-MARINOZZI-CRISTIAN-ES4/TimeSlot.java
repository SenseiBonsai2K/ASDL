...
public class TimeSlot implements Comparable<TimeSlot> {
    ...
    public int hashCode() {
		...
		return hash = 31 * hash + this.getStop().hashCode();
    }
    ...
    public int getMinutesOfOverlappingWith(TimeSlot o) {
		...
		else if ((this.getStart().after(o.getStart()) && this.getStop().before(o.getStop()))) //caso in cui il TimeSlot inizia e finisce mentre il parametro è in corso.
		...
		else if(this.compareTo(o)==0) //caso in cui i due TimeSlot iniziano e finiscono insieme.
			minutes = this.getStop().getTimeInMillis() - this.getStart().getTimeInMillis() / 60000;//trasformo in millisecondi per poi ricavare i minuti.
		else
			return -1;
    ...
    public String toString() {
        // TODO implementare - riutilizzare il codice della ES 3 o migliorarlo
    	return "[" + this.getStart().get(Calendar.DAY_OF_MONTH) + "/" + (this.getStart().get(Calendar.MONTH)+1) + "/" //concateno le stringhe per ottenere 
				...
				+ (this.getStop().get(Calendar.MONTH)+1) + "/" + this.getStop().get(Calendar.YEAR) + " "
