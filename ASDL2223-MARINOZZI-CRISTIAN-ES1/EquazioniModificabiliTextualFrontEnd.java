...
public class EquazioniModificabiliTextualFrontEnd {
	...
	public static void main(String[] args) {
		...
		while (!terminate) {
			...
			do {
				/*
				* va inserito il retry = false all'interno del do altrimenti si viene a creare un loop
				* il loop è dato dal while che continua la sua esecuzione finchè retry è true
				* ma non essendoci mai un assegnazione retry = false all'interno del do
				* la prima volta che retry prende il valore true rimmarrà così all'infinito.
				*/
			...
			if (eq == null)
				eq = new EquazioneSecondoGradoModificabileConRisolutore(a, b, c);
			else {
				eq.setA(a);
				eq.setB(b);
				eq.setC(c);
